...
inline long get_filesize(const char* filepath)
{
    if (!filepath)
    {
        return 0;
...
inline
int simple_inter_process_lock(const char *name)
{
    ...
    if (fd != -1)
    {
        ...
        {
            return 0; ////create a inter-process lock;
        }
    }
    else
    {
...
public:
    ...
    ~PrintExcuTime()
    {
        ...
        if (!m_sz_log_file)
        {
            printf("execute time is : %ds, %dus\n", (int) (m_timeval_end.tv_sec - m_timeval_begin.tv_sec),
                   (int) (m_timeval_end.tv_usec - m_timeval_begin.tv_usec));
        }
        else
        {
